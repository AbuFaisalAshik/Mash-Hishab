    ...
    >
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <button
          type="button"
          onClick={onExportCsv}
          className="py-2 px-3.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold flex items-center gap-1.5 shadow-2xs cursor-pointer transition-all"
        >
          <IconRenderer name="Download" className="w-4 h-4 text-emerald-600" />
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-5 space-y-3 shadow-xs">
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-500 font-medium">
          <span>
            ...
            <strong className="text-slate-700">{formatMoney(summary.totalAvailable, lang)}</strong>
          </span>
          <span>
            {lang === 'bn' ? 'অবশিষ্ট:' : 'Remaining:'}{' '}
            <strong className="text-emerald-700">{formatMoney(summary.remainingBalance, lang)}</strong>
          </span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-5 space-y-4 shadow-xs">
        <h2 className="text-sm font-bold text-slate-900 flex items-center gap-2">
          <IconRenderer name="PieChart" className="w-4 h-4 text-emerald-600" />
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-5 space-y-4 shadow-xs">
        <h2 className="text-sm font-bold text-slate-900 flex items-center gap-2">
          <IconRenderer name="TrendingUp" className="w-4 h-4 text-teal-600" />
